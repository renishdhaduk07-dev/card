...
def _save_logo_image(context, image_url: str) -> None:
    ...
    try:
        ...
        print(f"[Research] ⚠️  Could not save logo as PNG (might be SVG): {e}")
...
def _get_company_description(page) -> str | None:
    ...
    1. meta description        → most reliable, written for SEO
    2. og:description          → written for social sharing
    3. twitter:description     → fallback social tag
    4. First meaningful <p>    → last resort from page content
    """

    # Priority 1 + 2 + 3 — meta tags
    for selector in [
...
def _extract_colors(image_path: str) -> tuple[str | None, str | None]:
    try:
        ...
        print(f"[Research] ⚠️  Color extraction failed: {e}")
        return None, None
...
def _run_playwright(url: str) -> dict:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Retry if Cloudflare challenge or error
            if (
                not og_image_url
            ...
            if logo_url:
                try:
                    ...
                    if LOGO_IMAGE_PATH.exists():
                        primary_color, secondary_color = _extract_colors(
                            str(LOGO_IMAGE_PATH)
                        )
                except Exception as e:
                    print(f"[Research] ⚠️  Logo fetch error: {e}")

            # OG image → save → extract colors
            if og_image_url:
                try:
                    _save_og_image(context, og_image_url)
                    # Priority 2: Extract colors from OG Image (if not found in Logo)
                    if not primary_color and OG_IMAGE_PATH.exists():
                        primary_color, secondary_color = _extract_colors(
                            str(OG_IMAGE_PATH)
                        )
                except Exception as e:
                    print(f"[Research] ⚠️  OG image error: {e}")

            # Fallback: extract colors from screenshot
            # Priority 3: Extract from Screenshot (if not found in Logo or OG Image)
            ...
            print(f"[Research] ❌ Playwright error: {e}")
        finally:
            browser.close()
...
def research_agent(state: CardState) -> CardState:
    ...
    missing = []
    if not result["logo_url"]:     missing.append("logo_url")
    if not result["primary_color"]: missing.append("primary_color")
    if not result["company_name"]:  missing.append("company_name")
